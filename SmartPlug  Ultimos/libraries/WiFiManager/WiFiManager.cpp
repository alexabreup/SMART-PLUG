...
extern String generateRandomString(int length); //  Chamada externa do main.cpp
...
void WiFiManager::handleWifi(boolean scan) {
    ...
    #endif
    handleRequest();
    String page = getHTTPHead(FPSTR(S_titlewifi)); // Cabeçalho da página
    ...
    // Adiciona o SSID e a senha gerados como sugestões
    pitem = FPSTR(HTTP_FORM_WIFI);
    pitem.replace(FPSTR(T_v), randomSSID);  // Campo de SSID sugerido
    pitem.replace(FPSTR(T_p), randomPassword);  // Campo de senha sugerida
...
void WiFiManager::handleParam() {
    ...
    #endif
    handleRequest();
    String page = getHTTPHead(F("Parâmetros")); // Cabeçalho da página

    // Corpo da página de parâmetros
    page += F("<h3>Configure os parâmetros aqui</h3>");
    page += FPSTR(HTTP_FORM_END);
...
void WiFiManager::handleParamSave() {
    ...
    #endif
    doParamSave();  // Salva os parâmetros se `doParamSave` já estiver implementada

    String page = getHTTPHead(F("Parâmetros Salvos"));
    page += F("<h3>Parâmetros foram salvos com sucesso!</h3>");
    page += FPSTR(HTTP_FORM_END);
